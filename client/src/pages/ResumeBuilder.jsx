        ...
        return (
          <div className="animate-slide-in">
            ...
            {education.map((edu, idx) => (
              <div
                key={idx}
                className="relative border border-slate-200 rounded-xl p-4 mb-4 bg-white/50 backdrop-blur-sm transition-all hover:shadow-md"
              >
                {idx > 0 && (
                  <button
                    type="button"
            ...
            <button
              type="button"
              onClick={() => addItem(setEducation, education)}
              className="text-indigo-600 flex items-center gap-1 hover:gap-2 transition-all"
            >
      ...
      case 3:
        return (
          <div className="animate-slide-in">
            ...
            {experience.map((exp, idx) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  />
                  <textarea
                    placeholder="Key responsibilities (raw)"
            ...
            <button
              type="button"
              onClick={() => addItem(setExperience, experience)}
              className="text-indigo-600 flex items-center gap-1 hover:gap-2 transition-all"
            >
      ...
      case 4:
        return (
          <div className="animate-slide-in">
            ...
            {projects.map((proj, idx) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-3">
                  ...
                  />
                  <textarea
                    placeholder="Description / your contributions"
            ...
            <button
              type="button"
              onClick={() => addItem(setProjects, projects)}
              className="text-indigo-600 flex items-center gap-1 hover:gap-2 transition-all"
            >
              <FiPlus /> Add Project
            </button>
          ...
          <div className="space-y-4 animate-slide-in">
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-5 rounded-xl border border-indigo-100">
              <p className="text-sm text-indigo-800 flex items-start gap-2">
                ...
                Once you generate, AI will enhance your experiences and projects
                into STAR‑format bullet points and create a professional
                summary.
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between mb-10">
          {[1, 2, 3, 4, 5].map((num) => (
            <div key={num} className="flex items-center">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all duration-300 ${step >= num ? "bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-md" : "bg-slate-100 text-slate-500"}`}
              >
                {num}
              </div>
              {num < 5 && (
                <div
                  className={`w-12 h-1 mx-2 rounded-full transition-all duration-500 ${step > num ? "bg-indigo-600" : "bg-slate-200"}`}
                />
              )}
            </div>
          ))}
            ...
            <div className="flex justify-between mt-10 pt-4 border-t border-slate-100">
              {step > 1 && (
                <button
                  type="button"
                  onClick={prevStep}
                  className="px-6 py-2.5 rounded-xl border border-slate-300 text-slate-700 hover:bg-slate-50 hover:border-indigo-300 transition-all flex items-center gap-2"
                >
              ...
              {step < 5 ? (
                <button
                  type="button"
                  onClick={nextStep}
                  className={`px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl shadow-md hover:shadow-lg transition-all flex items-center gap-2 ${step === 1 ? "ml-auto" : ""}`}
                >
